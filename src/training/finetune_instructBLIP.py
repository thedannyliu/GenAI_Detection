...
class AINatureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Create proper labels for training
        input_ids = processed_inputs["input_ids"]
        
        # Tokenize just the answer to find its position
        ...
        # Find the answer tokens in the sequence and mask everything else
...
def compute_accuracy_metrics(eval_preds):
    """Compute accuracy for the model predictions."""
    ...
    accuracy = correct_predictions.sum() / valid_positions.sum()
...
class InstructBLIPTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        """
        Compute loss with proper handling of InstructBLIP outputs.
        Updated to support the new num_items_in_batch parameter in recent transformers versions.
        """
        ...
        with torch.no_grad():
            if has_labels:
                ...
                if isinstance(outputs, dict):
                    # Clean outputs to remove cache objects
                    cleaned_outputs = {}
                    for key, value in outputs.items():
                        if key not in ignore_keys and not key.endswith('_cache'):
                            # Only keep tensor outputs that can be properly processed
...
def main():
    ...
    training_args = TrainingArguments(
        ...
        # Additional settings to avoid cache issues
        include_inputs_for_metrics=False,  # Important: avoid passing inputs to metrics
    )
